#!/usr/bin/env python3
"""
Add Port 8080 Rule to Security Group
This script adds a new rule for port 8080 with a specific description format
"""
...
def add_port_rule(security_group_id, ip_address, port, access_name, username):
    """
    Add a new port rule to a security group with formatted description
    ...
    try:
        ...
        for rule in sg.get('IpPermissions', []):
            if rule.get('FromPort') == port and rule.get('ToPort') == port:
                for ip_range in rule.get('IpRanges', []):
                    if ip_range.get('CidrIp') == ip_address:
                        print(f"Confirmed: Rule added for port {port}")
                        print(f"Description: {ip_range.get('Description')}")
                        return True
        
        print("WARNING: Could not verify the rule was added")
        return False
...
def main():
    ...
    print("=== Add Port 8080 Rule to Security Group ===")
    ...
    success = add_port_rule(security_group_id, ip_address, port, access_name, username)
    
    if success:
        print("\nSUCCESS: Port 8080 rule has been processed!")
        sys.exit(0)
    else:
        print("\nFAILED: Could not process port 8080 rule!")
        sys.exit(1)
